...
export function registerCreateCommand(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('celer.create', async () => {
            const createType = await vscode.window.showQuickPick(
                [
                    { label: 'Platform', value: 'platform', description: 'Create a new platform configuration' },
            ...
            if (name) {
                try {
                    await celer.create(createType.value, name);
